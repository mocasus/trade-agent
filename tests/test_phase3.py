...
from trade_agent.models import Action, Candle, Decision, Indicators, MarketContext, OCOOrder, PartialExit, Position, Ticker

def _ticker(price=100.0):
    return Ticker(symbol="BTC", last_price=price, bid=price-0.1, ask=price+0.1, volume_24h=1000, change_pct_24h=0)

def _ctx(price=100.0, positions=None):
    return MarketContext(symbol="BTC", timeframe="1h", candles=[], ticker=_ticker(price),
                         indicators=Indicators(), positions=positions or [])

def _pos(entry=100.0, amount=1.0):
    return Position(symbol="BTC", side="long", entry_price=entry, amount=amount, current_price=entry)
